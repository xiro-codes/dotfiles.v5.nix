...
def generate_configs(args):
    ...
    shutil.copytree(src_repo, dest_repo, dirs_exist_ok=True, 
    ...
    with open("templates/host-home.nix", "r") as f:
        content = f.read().replace("TEMPLATE_USER", args.user).replace("TEMPLATE_DESKTOP", args.desktop)
    ...
    # 3. Hardware Scan
    run_cmd(["nixos-generate-config", "--root", "/mnt", "--dir", f"{sys_dir}/hardware-configuration.nix"])
...
def main():
    ...
    if input("⚠️  Proceed? (y/N): ").lower() != 'y': sys.exit(0)
